...
<template>
  <header class="h-16 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-4 sm:px-6 bg-white/80 dark:bg-[#050C16]/80 backdrop-blur top-0 sticky z-10 transition-colors duration-300 w-full">
    <!-- Mobile Menu Toggle -->
    ...
    <div class="flex items-center gap-3 sm:gap-6 ml-auto">
      ...
      <UDropdownMenu :items="profileItems" :ui="{ content: 'w-48' }">
        <div class="flex items-center gap-3 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800/50 p-1.5 rounded-lg transition-colors">
          ...
          <UIcon name="i-lucide-chevron-down" class="hidden sm:block w-4 h-4 text-slate-400" />
        </div>
      </UDropdownMenu>
    </div>
  </header>
